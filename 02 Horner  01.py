...
def printEkv(degree):
    c = degree                      #utskrift av ekvationen!
    for i in range(degree,-1,-1):               
        print(ekv[i],"x",c,"   + ",end='')
        c    -= 1 
        if c < 0:
...
def find_start(ekv_z,degree,start2,y_noll):
    ...
    if degree == 2 or degree == 3 or degree == 4 or degree == 5:
        ...
        if degree == 4:
            ...
            h_noll *= 0.001     #  Här kan man ändra känsligheten av algoritmen!
                                # h_noll ska multipliceras med 1.0 (0.75)
        ...
        if degree == 5:
            ...
            ftal3  = num_1 * ftal
            num_2  += ftal3   # andragrads termen borde vara färdig!
            ftal2  = num_2 * ftal
            num_3  += ftal2   # tredjegrads termen borde vara färdig
            #print("num_0, 1..num 5 =   ",num_0," ",num_1," ",num_2," ",num_3," ",num_4," ",num_5)
...
def find_z(ekv_z,copy,degree,start):
    ...
    print("The equation in z = ",ekv_z,end=' ')                  # Jag är här!
    ekv_z.reverse()     # vänder termerna för bra utskrift!
    return ekv_z
